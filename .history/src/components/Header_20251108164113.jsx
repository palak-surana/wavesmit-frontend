...
const Navbar = () => {
  ...
  const [dropdownOpen, setDropdownOpen] = useState(null); // "events" | "report" | null
  ...
  // IMPORTANT: keep these paths lowercase and matching your App.jsx routes
  const Links = [
    { link: "/" },            // Home
    { link: "/about" },       // About
    { link: "/vision" },      // Vision
    { link: "/events" },      // Events (has dropdown)
    { link: "/gallery" },     // Gallery
    { link: "/nss" },         // NSS (if you use a page)
    { link: "/awards" },      // Awards
    { link: "/reports" },     // Reports (has dropdown)
    ...
    <div className="w-screen fixed top-0 left-0 right-0 z-50">
      ...
      >
        ...
        <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center justify-center">
          <img src="/assets/mitAoe.jpg" alt="MITAOE Logo" className="h-20 md:h-24 object-contain" />
        </div>
        ...
        <div className="flex flex-col items-end space-y-2">
          <div className="flex space-x-3">
            <a
              ...
              className="w-6 h-6 bg-gradient-to-r from-pink-600 to-purple-600 hover:scale-110 rounded-full flex items-center justify-center transition-all duration-300"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-6 h-6 bg-red-600 hover:bg-red-700 hover:scale-110 rounded-full flex items-center justify-center transition-all duration-300"
            >
              <FaYoutube className="text-white text-lg" />
            </a>
          </div>

          <button onClick={toggleLanguage} className="bg-[#1a237e] text-white px-1 py-1 rounded-md text-sm hover:bg-[#303f9f] transition">
        ...
        <div className="max-w-[2000px] w-full flex items-center justify-between px-4 md:px-10">
          ...
          <ul className="flex-1 flex flex-wrap items-center justify-center gap-6 relative">
            {Links.map((item, index) => {
              const label = translations[language].links[index] ?? translations[language].links[0];

              // Events dropdown (use item.link to identify)
              if (item.link === "/events") {
                return (
                  <li
                    key={item.link}
                    className="relative group flex items-center"
                    onMouseEnter={() => setDropdownOpen("events")}
                  ...
                  >
                    <Link
                      ...
                      className={`text-white font-semibold text-[15px] uppercase tracking-wide px-3 py-2 flex items-center hover:text-yellow-400 transition-all duration-200 ${
                        location.pathname.startsWith("/events") ? "border-b-2 border-yellow-400" : "border-b-2 border-transparent"
                      }`}
                    >
                      {label}
                    </Link>
              ...
              // Reports dropdown (use item.link)
              if (item.link === "/reports") {
                return (
                  <li
                    key={item.link}
                    className="relative group flex items-center"
                    onMouseEnter={() => setDropdownOpen("report")}
                    onMouseLeave={() => setDropdownOpen(null)}
                  >
                    <Link
                      to={item.link}
                      className={`text-white font-semibold text-[15px] uppercase tracking-wide px-3 py-2 flex items-center hover:text-yellow-400 transition-all duration-200 ${
                        location.pathname.startsWith("/reports") ? "border-b-2 border-yellow-400" : "border-b-2 border-transparent"
                      }`}
                    >
                      {label}
                    </Link>
                ...
                <li key={item.link} className="flex items-center">
                  <Link
                    ...
                    className={`text-white font-semibold text-[15px] uppercase tracking-wide px-3 py-2 hover:text-yellow-400 transition-all duration-200 ${
                      location.pathname === item.link ? "border-b-2 border-yellow-400" : "border-b-2 border-transparent"
                    }`}
                  >
                    {label}
                  </Link>
        ...
        <>
          ...
          <div className="fixed top-[150px] right-0 w-64 h-[calc(100vh-150px)] bg-white z-50 shadow-lg overflow-y-auto">
            <div className="p-4 space-y-2">
              {Links.map((item, index) => (
                <Link key={item.link} to={item.link} onClick={() => setIsOpen(false)} className="block text-black px-3 py-2 rounded hover:bg-gray-100">
